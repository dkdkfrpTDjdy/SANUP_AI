...
5. 단순 증감 나열이 아니라 "팀-현장-고객사-세부항목" 연결 구조를 보여줘라.
...
- 가능하면 같은 팀이 여러 현장에서 반복 등장하는지 확인하고,
  복수 현장 기반이면 "어떤 현장군이 팀 실적의 기반인지"를 설명한다.
- 단, payload에 없는 현장 수를 임의로 만들지 말고, 제공된 상위 현장 기록 범위 안에서만 해석한다.
...
- 아래 패턴을 우선 탐색:
  ...
  6) 겉으로는 총액만 보이지만 실제로는 특정 현장군이 실적 결을 만드는 패턴
- creative_lines 중 최소 1개는 반드시 "성과 좋은 팀이 어떤 현장군을 기반으로 좋은 성과를 내는지"를 구조적으로 해석해야 한다.
- 팀에 대한 부정적 평가 금지

item == "그외" 추가 규칙:
- 최소 2개 문장에서 반드시 아이템 세분화를 명시
- "그외"라고만 쓰지 말고 실제 아이템 세분화명을 넣어라
- 세부항목이 어떤 방식으로 실적의 중심을 만드는지 설명하라
