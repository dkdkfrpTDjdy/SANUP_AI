...
6. 같은 유형의 문장을 반복하지 말고, 각 문장은 다른 관찰 포인트를 가져야 한다.
...
[A. item == "그외"]
- 반드시 payload.top_detail_current 또는 payload.top_detail_up를 사용하라.
- "그외"에서는 아이템 세분화가 핵심이다.
- 최소 3개 문장 중 2개 이상에서 실제 "아이템 세분화"명을 직접 써야 한다.
- 가능하면 6개 문장 중 3개 이상에서 실제 세분화명을 직접 써라.
- "그외"라는 표현만으로 끝내지 말고, 실제 세분화 항목명(예: COMP 등)을 반드시 넣어라.
- 어떤 세분화 항목이 렌탈료가 큰지, 어떤 세분화 항목이 임대대수가 큰지, 어떤 세분화 항목이 구조의 중심인지 드러내라.
- 적어도 1개 문장은 "당월 세부항목 렌탈료" 기준 상위 항목을 써야 한다.
- 적어도 1개 문장은 "당월 세부항목 임대대수" 또는 "세부항목 내 비중"을 써야 한다.
- 만약 top_detail_current / top_detail_up에 세분화명이 있으면, 세분화명을 쓰지 않은 문장은 실패로 간주하라.
- "그외" 아이템에서 세분화명을 빼고 일반론만 쓰는 것은 금지한다.
...
- 최소 1개 문장에서는 "FL" 또는 "FL 물류"를 직접 명시하라.
- 가능하면 "FL"과 "FL 물류" 중 어떤 구성이 더 중심인지 1개 이상 문장에 반영하라.
...
추가 강제 규칙:
- item == "그외"이면 creative_lines 3개 중 최소 2개는 실제 세분화명을 포함해야 한다.
- item == "그외"이면 quant_lines 3개 중 최소 1개도 실제 세분화명을 포함해야 한다.
- item == "FL"이면 가능하면 "FL" / "FL 물류" 구분을 최소 1개 문장에 반영하라.
...
- item == "그외"일 때 실제 아이템 세분화명이 quant_lines/creative_lines에 충분히 들어갔는가?
...
- item == "그외" 이면 quant_lines 중 최소 1개, creative_lines 중 최소 2개 문장에서 실제 아이템 세분화명을 명시
- item == "그외" 이면 top_detail_current 또는 top_detail_up에 있는 실제 세분화명을 사용
- item == "AWP" 이면 아이템 세분화 언급 금지
- item == "FL" 이면 가능하면 "FL" 또는 "FL 물류"를 최소 1개 문장에 반영
""".strip(),
            )
